...
export const updateBillStatus = async (req: Request, res: Response) => {
    try {
        ...
        if (status === "paid") {
            ...
            const bill = await Bill.findOne({ _id: id, userId });
            if (bill && bill.isRecurring && bill.billFrequency !== "one-time") {
                const nextDueDate = calculateNextDueDate(bill.dueDate, bill.billFrequency);
                updateData.nextDueDate = nextDueDate;
                updateData.dueDate = nextDueDate;
            }
        }

        const updatedBill = await Bill.findOneAndUpdate({ _id: id, userId }, updateData, { new: true });
...
export const getBillStats = async (req: Request, res: Response) => {
    try {
        ...
        const [totalBills, unpaidBills, overdueBills, upcomingBills] = await Promise.all([
