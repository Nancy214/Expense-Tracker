...
import { User, Settings, LogOut, Camera, Save, Edit3, Shield, TrendingUp } from "lucide-react";
...
const ProfilePage: React.FC = () => {
    ...
    const filteredCountryOptions = useMemo(
        () => countryOptions.filter((country) => country.toLowerCase().includes(countrySearch.toLowerCase())),
        [countrySearch]
    );
    ...
    const fetchCountryTimezoneCurrency = async () => {
        ...
        const timezonesOptions = response.map((item) => item.timezones).flat();
