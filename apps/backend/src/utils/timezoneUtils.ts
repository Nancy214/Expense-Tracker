...
export const getCurrentTimeInTimezone = (timezone: string): Date => {
	...
	try {
		...
		const resultDate = new Date(parseInt(year), parseInt(month) - 1, parseInt(day), parseInt(hours), parseInt(minutes), parseInt(seconds));
...
export const convertUserTimezoneToUTC = (userDate: Date, timezone: string): Date => {
	...
	try {
		...
		const userString = userDate.toLocaleString("en-US", {
			timeZone: ianaTimezone,
		});
