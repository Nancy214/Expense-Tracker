		...
		const tokens: string = encodeURIComponent(
			JSON.stringify({
				...
				user: {
					...
					dateOfBirth:
						(nestedUser?.dateOfBirth || flat?.dateOfBirth) && isValid(new Date(nestedUser?.dateOfBirth || flat?.dateOfBirth))
							? format(new Date(nestedUser?.dateOfBirth || flat?.dateOfBirth), "dd/MM/yyyy")
							: "",
