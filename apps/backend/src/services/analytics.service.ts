...
export class AnalyticsService {
    ...
    private async getDailyTransactionsForMonth(
        userId: string,
        year: number,
        month: number
    ...
    async getBillsCategoryBreakdown(
        userId: string,
        period: Period = Period.MONTHLY,
        subPeriod?: string
        ...
        if (subPeriod && typeof subPeriod === "string") {
            ...
            switch (period) {
                case Period.MONTHLY: {
                    ...
                    const dailyData = await this.getDailyTransactionsForMonth(
                        userId,
                ...
                case Period.QUARTERLY: {
                    ...
                    for (let month = quarterStartMonth; month < quarterStartMonth + 3; month++) {
                        ...
                        const monthData = await this.getTransactionsForPeriod(
                            userId,
            ...
            for (let i = 5; i >= 0; i--) {
                ...
                const monthData: SpecificPeriodIncomeExpenseData = await this.getTransactionsForPeriod(
                    userId,
                    monthDates.startDate,
                    monthDates.endDate
                );
    ...
    async getMonthlySavingsTrend(
        ...
        if (subPeriod && typeof subPeriod === "string") {
            ...
            switch (period) {
                case Period.MONTHLY: {
                    ...
                    const dailySavingsData = await this.getDailyTransactionsForMonth(
                        userId,
        ...
        const bestMonth: MonthlySavingsData = activeMonthsData.reduce(
            (best, current) => (current.savings > best.savings ? current : best),
            activeMonthsData[0]
        );
        const worstMonth: MonthlySavingsData = activeMonthsData.reduce(
            (worst, current) => (current.savings < worst.savings ? current : worst),
            activeMonthsData[0]
        );
