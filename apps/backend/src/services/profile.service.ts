...
export class ProfileService {
	...
	async getProfile(userId: string) {
		...
		if (userDoc.profilePicture) {
			const getCommand: GetObjectCommand = new GetObjectCommand({
				Bucket: AWS_BUCKET_NAME,
				Key: userDoc.profilePicture.startsWith("profile-pictures/") ? userDoc.profilePicture : `profile-pictures/${userDoc.profilePicture}`,
			});
	...
	async updateProfile(userId: string, profileData: ProfileData, file?: Express.Multer.File) {
		...
		if (updatedUser.profilePicture) {
			const getCommand: GetObjectCommand = new GetObjectCommand({
				Bucket: AWS_BUCKET_NAME,
				Key: updatedUser.profilePicture.startsWith("profile-pictures/") ? updatedUser.profilePicture : `profile-pictures/${updatedUser.profilePicture}`,
			});
			profilePictureUrl = await getSignedUrl(s3Client, getCommand, {
				expiresIn: 300,
			}); // 5 minutes
	...
	async updateSettings(userId: string, settingsData: SettingsData) {
		const { monthlyReports, expenseReminders, billsAndBudgetsAlert, expenseReminderTime }: SettingsData = settingsData;
