...
export class BudgetDAO {
    ...
    static detectBudgetChanges(oldBudget: BudgetFormData, newBudgetData: BudgetFormData): BudgetChange[] {
        ...
        if (oldBudget.title !== newBudgetData.title) {
            changes.push({ field: "title", oldValue: oldBudget.title, newValue: newBudgetData.title });
        }
        if (oldBudget.amount !== newBudgetData.amount) {
            changes.push({ field: "amount", oldValue: oldBudget.amount, newValue: newBudgetData.amount });
        }
        ...
        if (oldBudget.category !== newBudgetData.category) {
            changes.push({ field: "category", oldValue: oldBudget.category, newValue: newBudgetData.category });
        }
