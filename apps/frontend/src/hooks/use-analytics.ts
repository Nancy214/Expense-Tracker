...
export const ANALYTICS_QUERY_KEYS = {
	expenseBreakdown: (period?: string, subPeriod?: string) => ["analytics", "expense-breakdown", period, subPeriod] as const,
...
export function useExpenseCategoryBreakdown(period?: string, subPeriod?: string): UseQueryResult<ExpenseCategoryBreakdownResponse, Error> {
...
export function useBillsCategoryBreakdown(period?: string, subPeriod?: string): UseQueryResult<BillsCategoryBreakdownResponse, Error> {
	const { isAuthenticated } = useAuth();
...
export function useIncomeExpenseSummary(period?: string, subPeriod?: string): UseQueryResult<IncomeExpenseSummaryResponse, Error> {
	const { isAuthenticated } = useAuth();
...
export function useMonthlySavingsTrend(period?: string, subPeriod?: string): UseQueryResult<MonthlySavingsTrendResponse, Error> {
	const { isAuthenticated } = useAuth();
...
export const transformExpensesToHeatmapData = (expenses: Transaction[]): HeatmapData[] => {
	const groupedByDate = expenses.reduce(
		(acc: Record<string, GroupedExpenseData>, expense: Transaction) => {
			let dateStr: string;
	...
	const sortedData = Object.values(groupedByDate).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
