import type { BudgetFormData, BudgetParams, BudgetProgressResponse, BudgetReminder, BudgetType } from "@expense-tracker/shared-types/src";
import { type UseMutationResult, type UseQueryResult, useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useAuth } from "@/context/AuthContext";
import { createBudget, deleteBudget, getBudgetProgress, getBudgets, processBudgetReminders, updateBudget } from "../services/budget.service";
...
export const useBudgets = (): UseBudgetsReturn => {
	...
	const budgetReminders: BudgetReminder[] = budgetProgressQuery.data ? processBudgetReminders(budgetProgressQuery.data) : [];
	...
	const updateBudgetMutation: UseMutationResult<BudgetType, Error, { id: string; budgetData: BudgetFormData }> = useMutation({
		mutationFn: ({ id, budgetData }: { id: string; budgetData: BudgetFormData }) => updateBudget(id, budgetData),
		onSuccess: () => {
			queryClient.invalidateQueries({
				queryKey: ["budgets"],
				exact: true,
	...
	return {
		...
		// Mutation functions
		createBudget: isAuthenticated ? (budgetData: BudgetFormData) => createBudgetMutation.mutateAsync(budgetData) : () => Promise.reject(new Error("Not authenticated")),
		updateBudget: isAuthenticated
			? ({ id, budgetData }: { id: string; budgetData: BudgetFormData }) => updateBudgetMutation.mutateAsync({ id, budgetData })
			: () => Promise.reject(new Error("Not authenticated")),
		deleteBudget: isAuthenticated ? (params: BudgetParams) => deleteBudgetMutation.mutateAsync(params) : () => Promise.reject(new Error("Not authenticated")),
