...
export function useDeleteOperations({ onRefresh, onBudgetProgressRefresh, onBudgetRemindersRefresh }: UseDeleteOperationsProps = {}): DeleteOperationsReturn {
	...
	const confirmBudgetDelete = useCallback(async (): Promise<void> => {
		...
		try {
			...
			const errorMessage = error && typeof error === "object" && "message" in error ? (error as { message: string }).message : "Failed to delete budget";
			toast({
				title: "Error",
				description: errorMessage,
				variant: "destructive",
	...
	}, [budgetToDelete, deleteBudget, queryClient, toast, onRefresh, onBudgetProgressRefresh, onBudgetRemindersRefresh]);
