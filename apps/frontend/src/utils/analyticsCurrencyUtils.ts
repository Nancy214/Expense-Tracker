...
export const groupExpenseCategoryBreakdownByCurrency = (
    transactions: Transaction[],
    period: Period,
    subPeriod: string
): Record<string, HorizontalBarData[]> => {
...
export const groupBillsCategoryBreakdownByCurrency = (
    transactions: Transaction[],
    period: Period,
    subPeriod: string
): Record<string, HorizontalBarData[]> => {
...
export const groupIncomeExpenseDataByCurrency = (
    transactions: Transaction[],
    period: Period,
    subPeriod: string
    ...
    filteredTransactions.forEach((t) => {
        ...
        if (period === Period.MONTHLY) {
            ...
            dateKey = `${String(transactionDate.getDate()).padStart(2, "0")}/${String(
...
export const groupSavingsTrendDataByCurrency = (
    transactions: Transaction[],
    period: Period,
    subPeriod: string
): Record<string, AreaChartData[]> => {
    const filteredTransactions = filterTransactionsByPeriod(transactions, period, subPeriod);
...
export const groupComparisonDataByCurrency = (
    transactions: Transaction[],
    period: Period,
    subPeriod: string
): Record<string, ComparisonLineData[]> => {
        ...
        currencySet.forEach((currency) => {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
                ...
                const currentDayExpenses = currentPeriodTransactions
                    .filter(
                        (t) =>
                            (t.currency || "INR") === currency &&
                            t.type === TransactionType.EXPENSE &&
                            format(parseTransactionDate(t.date), "dd/MM") === currentDayStr
                    )
                ...
                const previousDayExpenses = previousPeriodTransactions
                    .filter(
                        (t) =>
                            (t.currency || "INR") === currency &&
                            t.type === TransactionType.EXPENSE &&
                            format(parseTransactionDate(t.date), "dd/MM") === previousDayStr
                    )
                    .reduce((sum, t) => sum + (t.amount || 0), 0);
