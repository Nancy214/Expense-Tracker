				...
				render={({ field }) => (
					<>
						<div className="flex items-center justify-between">
							...
							{maxLength && (
								<span className={cn("text-xs", isAtLimit ? "text-red-500 font-medium" : "text-gray-500")}>
						...
						<motion.div
							...
							animate={{
								opacity: error || isAtLimit ? 1 : 0,
								y: error || isAtLimit ? 0 : -10,
							}}
							transition={{ duration: 0.3 }}
						>
							{error && <p className="text-xs text-red-500">{error.message as string}</p>}
							{isAtLimit && !error && <p className="text-xs text-orange-500">Character limit reached ({maxLength} characters)</p>}
						</motion.div>
					</>
				)}
			/>
