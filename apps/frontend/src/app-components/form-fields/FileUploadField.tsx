...
export const FileUploadField: React.FC<FileUploadFieldProps> = ({
	...
	const files: File[] = multiple ? (Array.isArray(fieldValue) ? fieldValue.filter((f) => f instanceof File) : []) : fieldValue instanceof File ? [fieldValue] : [];
		...
		return (
			<>
				<input
					...
					onChange={(e) => {
						if (e.target.files) {
							...
							const validFiles = selectedFiles.filter((file) => file.type.startsWith("image/") || file.type === "application/pdf");
				...
				<label
					...
					className={cn("cursor-pointer text-gray-500 hover:text-blue-500 transition-colors block h-10 flex items-center", disabled && "opacity-50 cursor-not-allowed")}
			...
			{!hideIcon && (
				<div className="flex items-end">
					<div className="relative pb-1">
						...
						<label
							htmlFor={`${name}-upload`}
							className={cn("cursor-pointer text-gray-500 hover:text-blue-500 transition-colors block", disabled && "opacity-50 cursor-not-allowed")}
							title={multiple ? "Upload receipts" : "Upload receipt"}
						>
							...
							<div className="group flex items-center gap-1 p-1 rounded border border-green-200 bg-green-50 hover:border-green-300 transition-all duration-200 mb-1 max-w-full overflow-hidden">
								...
								<div className="flex-1 min-w-0 overflow-hidden">
									<div className="flex items-center gap-1 min-w-0">
										...
										<Badge variant="secondary" className="text-xs px-1 py-0 bg-green-100 text-green-700 border-green-200 flex-shrink-0">
							...
							{files.map((file: File, idx: number) => {
								...
								return (
									...
									>
										...
										<div className="flex-1 min-w-0 overflow-hidden">
											<div className="flex items-center gap-1 min-w-0">
												<p className="text-xs font-medium text-gray-900 truncate flex-1">{file.name}</p>
												...
												<Button
													...
													onClick={() => {
														if (multiple) {
															const updatedFiles = files.filter((_: File, i: number) => i !== idx);
