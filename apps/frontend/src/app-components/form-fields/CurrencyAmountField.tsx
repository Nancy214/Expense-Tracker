	...
	return (
		<div className={cn("space-y-2", className)}>
			...
			<div className="flex">
						...
						{currencyOptions
							...
							.map((option) => (
								...
								>
									...
									{option.name && <span className="text-xs text-muted-foreground ml-2">{option.name}</span>}
								</DropdownMenuItem>
							))}
					</DropdownMenuContent>
				</DropdownMenu>
				...
				<div className="relative flex-1">
					<Input
						...
						max={max}
						step={step}
						className={cn("h-8 rounded-l-none focus:z-10", showUploadIcon && "pr-9", error && "border-red-500 focus:border-red-500 focus:ring-red-500")}
						{...register(amountName, {
							valueAsNumber: true,
							onBlur: () => {
								trigger(amountName);
			...
			<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: error ? 1 : 0, y: error ? 0 : -10 }} transition={{ duration: 0.3 }}>
				...
				{!amountError && currencyError && <p className="text-xs text-red-500">{currencyError.message as string}</p>}
			</motion.div>
		</div>
	);
};
