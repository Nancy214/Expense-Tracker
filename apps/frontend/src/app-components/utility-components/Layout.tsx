...
function LayoutContent({ children }: LayoutProps) {
	...
	const isAuthPage = ["/", "/login", "/register", "/forgot-password", "/reset-password", "/auth/google/callback", "/logout", "/onboarding"].includes(location.pathname);
							...
							<DropdownMenuTrigger asChild>
								<Button variant="ghost" className="relative h-8 w-8 rounded-full">
									<Avatar className="h-8 w-8">
										...
										<AvatarFallback className="bg-gradient-to-br from-blue-500 to-purple-600 text-white">{user?.name?.charAt(0) || "U"}</AvatarFallback>
									</Avatar>
								</Button>
							</DropdownMenuTrigger>
					...
					return (
						<Link
							...
							className={cn(
								...
								isActive ? "text-blue-700 scale-[1.02]" : "text-gray-700 hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-gray-100/50 hover:text-gray-900"
							)}
							style={{
						...
						>
							...
							<Icon
								className={cn(
									...
									isActive ? "text-blue-700 scale-110" : "text-gray-600 group-hover:text-gray-900 group-hover:scale-110"
								)}
							/>
							...
							<span
								className={cn(
									...
									isActive ? "font-semibold text-blue-700 translate-x-1" : "group-hover:translate-x-1 group-hover:font-medium"
								)}
							>
								{label}
							</span>
							...
							{isActive && <div className="absolute inset-0 bg-blue-400/5 rounded-lg animate-pulse z-10" />}
