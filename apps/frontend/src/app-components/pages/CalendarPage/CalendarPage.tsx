...
const CalendarPage: React.FC = () => {
	...
	const calendarEvents: CalendarEvent[] = expenses.map((expense: Transaction) => {
		...
		return {
			...
			extendedProps: {
				...
				recurringFrequency: "recurringFrequency" in expense ? (expense.recurringFrequency as string | undefined) : undefined,
			},
		};
	});
						...
						<div className="space-y-2">
							...
							<div className="space-y-1 text-xs">
								...
								<div className="flex justify-between">
									...
									<span className="font-medium">{format(new Date(eventInfo.event.start), "MMM dd, yyyy")}</span>
								</div>
							</div>
						</div>
					</TooltipContent>
	...
	const filteredEvents: CalendarEvent[] = useMemo(() => {
		return calendarEvents.filter((event) => {
			...
			const categoryAllowed: boolean = selectedCategories.size === 0 || selectedCategories.has(event.extendedProps.category);
	...
	const getLegendCategories = (type: TransactionType) => {
		...
		return Object.entries(colorMap).filter(([category]) => visibleCategories.size === 0 || visibleCategories.has(category));
	};
								...
								<div className="legend-content">
									<div className="legend-section">
										...
										<div className="legend-toggle-group">
											<button
												type="button"
												className={`legend-toggle legend-toggle-expense ${typeFilters[EXPENSE_TYPE] ? "active" : ""}`}
											...
											<button
												type="button"
												className={`legend-toggle legend-toggle-income ${typeFilters[INCOME_TYPE] ? "active" : ""}`}
									...
									<div className="legend-section legend-scroll">
										{getLegendCategories(EXPENSE_TYPE).length > 0 && (
											<div>
												<div className="legend-section-header">
													...
													<span className="legend-count">{getLegendCategories(EXPENSE_TYPE).length}</span>
												</div>
												<div className="legend-chip-group">
													{getLegendCategories(EXPENSE_TYPE).map(([category, color]: [string, string]) => {
										...
										{getLegendCategories(INCOME_TYPE).length > 0 && (
											<div>
												<div className="legend-section-header">
													...
													<span className="legend-count">{getLegendCategories(INCOME_TYPE).length}</span>
												</div>
												<div className="legend-chip-group">
													{getLegendCategories(INCOME_TYPE).map(([category, color]: [string, string]) => {
														const active = isCategoryActive(category);
														return (
															<button
																type="button"
										...
										{visibleCategories.size === 0 && <div className="legend-empty-state">No events in current view</div>}
									</div>
