...
const BudgetPage: React.FC = () => {
	...
	const handleEdit = (budget: BudgetType): void => {
		setPageState((prev: { isDialogOpen: boolean; editingBudget: BudgetType | null; dismissedReminders: Set<string> }) => ({
			...prev,
			...
			<div className="p-4 md:p-6 lg:p-4 space-y-6 max-w-full">
				...
				<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
					<div>
						...
						<p className="text-gray-600 dark:text-gray-400 mt-1">Manage your spending limits and track progress</p>
					</div>
					{budgets.length !== 0 ? (
					...
					<Card className="mb-6">
						...
						<CardContent>
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
									...
									{savingsByCurrency.length > 0 ? (
										<div className="space-y-1">
											{savingsByCurrency.map((saving) => (
												<div key={saving.currency} className="text-2xl font-bold text-green-600">
										...
										<>
											<div className="text-2xl font-bold text-green-600">{formatAmount(0, userCurrency)}</div>
								...
								{/* Days Until Reset */}
								<div className="text-center p-4 bg-muted/50 rounded-lg">
									<div className="text-2xl font-bold text-blue-600">{budgetProgress.daysUntilReset !== null ? budgetProgress.daysUntilReset : "N/A"}</div>
									<div className="text-sm text-muted-foreground">
										{budgetProgress.daysUntilReset === 1 ? "Day" : budgetProgress.daysUntilReset === 0 ? "Today" : "Days"}
									</div>
										...
										<Tooltip>
											<TooltipTrigger asChild>
												<div className="cursor-help">
													...
													<div className="text-sm text-muted-foreground">{budgetProgress.budgetHealth.label}</div>
											...
											<TooltipContent className="max-w-sm">
												<div className="space-y-2">
													<div className="font-semibold text-sm border-b pb-1">Health Score Breakdown</div>
													<div className="text-xs space-y-1">
														<div className="flex justify-between">
															<span>Base Score:</span>
															<span className="font-medium">{budgetProgress.budgetHealth.breakdown.baseScore}</span>
														</div>
														{budgetProgress.budgetHealth.breakdown.overBudgetCount > 0 && (
															<div className="flex justify-between text-red-600">
																...
																<span className="font-medium">-{budgetProgress.budgetHealth.breakdown.overBudgetPenalty}</span>
															</div>
														)}
														{budgetProgress.budgetHealth.breakdown.highUsageCount > 0 && (
															<div className="flex justify-between text-orange-600">
																...
																<span className="font-medium">-{budgetProgress.budgetHealth.breakdown.highUsagePenalty}</span>
															</div>
														)}
														{budgetProgress.budgetHealth.breakdown.mediumUsageCount > 0 && (
															<div className="flex justify-between text-yellow-600">
																<span>
																	Medium Usage 60-80% ({budgetProgress.budgetHealth.breakdown.mediumUsageCount}
																	):
																</span>
																<span className="font-medium">-{budgetProgress.budgetHealth.breakdown.mediumUsagePenalty}</span>
															</div>
														)}
														{budgetProgress.budgetHealth.breakdown.lowUsageCount > 0 && (
															<div className="flex justify-between text-green-600">
																...
																<span className="font-medium">+{budgetProgress.budgetHealth.breakdown.lowUsageBonus}</span>
															</div>
														)}
														{budgetProgress.budgetHealth.breakdown.perfectRecordBonus > 0 && (
															<div className="flex justify-between text-green-600">
																<span>Perfect Record Bonus:</span>
																<span className="font-medium">+{budgetProgress.budgetHealth.breakdown.perfectRecordBonus}</span>
															</div>
														)}
				...
				<AddBudgetDialog
					...
					onSuccess={() => {
						setPageState((prev: { isDialogOpen: boolean; editingBudget: BudgetType | null; dismissedReminders: Set<string> }) => ({ ...prev, editingBudget: null }));
					}}
				/>
				...
				{budgets.length === 0 ? (
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
						{budgets.map((budget: BudgetType) => {
							const progress: BudgetProgress | undefined = budgetProgress.budgets.find((p: BudgetProgress) => p.id === budget.id);
							return (
								<Card key={budget.id}>
									<CardHeader>
										<div className="flex justify-between items-start">
											<div>
												...
												<div className="text-md text-primary">{formatAmount(budget.amount, budget.currency)}</div>
												<CardDescription>
													{formatRecurrence(budget.recurrence as BudgetRecurrence)} Budget <br />
													Starts from: {formatToHumanReadableDate(budget.startDate, "EEE, MMM dd, yyyy")}
												</CardDescription>
											</div>
										...
										{progress && (
											<div className="space-y-2">
												...
												<div className="flex justify-between text-gray-500">
													<span className="text-sm text-primary">Spent: {formatAmount(progress.totalSpent, progress.currency)}</span>
													<span className="text-sm text-primary">Remaining: {formatAmount(progress.remaining, progress.currency)}</span>
												</div>
												<div className="text-xs text-gray-500">{progress.expensesCount} transactions this recurrence</div>
												{progress.isOverBudget && (
													<div className="flex items-center gap-1 text-xs text-red-500 font-medium" role="alert">
														<XCircle className="h-3 w-3" aria-hidden="true" />
														Over budget by {formatAmount(Math.abs(progress.remaining), progress.currency)}
													</div>
												)}
											</div>
										)}
					...
					<div className="mt-2">
						<div className="flex items-center justify-between">
							...
							<div className="flex items-center space-x-2">
								<Switch id="show-budget-history" checked={showBudgetHistory} onCheckedChange={setShowBudgetHistory} />
				...
				message={
					budgetToDelete
						? `Are you sure you want to delete the ${formatRecurrence(
							...
							)} budget of ${formatAmount(budgetToDelete.amount, budgetToDelete.currency)} for ${budgetToDelete.category}? This action cannot be undone.`
