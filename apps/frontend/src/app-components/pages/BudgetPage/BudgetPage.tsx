							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
								...
								<div className="text-center p-4 bg-muted/50 rounded-lg">
									<div className="text-2xl font-bold text-green-600">
										{formatAmount(budgetProgress.budgets.reduce((sum: number, b: BudgetProgress) => sum + b.amount, 0))}
									</div>
									<div className="text-sm text-muted-foreground">Total Budget</div>
								</div>
								<div className="text-center p-4 bg-muted/50 rounded-lg">
									<div className="text-2xl font-bold text-blue-600">
										{formatAmount(budgetProgress.budgets.reduce((sum: number, b: BudgetProgress) => sum + b.totalSpent, 0))}
									</div>
									<div className="text-sm text-muted-foreground">Total Spent</div>
								</div>
								<div className="text-center p-4 bg-muted/50 rounded-lg">
									<div className="text-2xl font-bold text-purple-600">{budgetProgress.budgets.filter((b: BudgetProgress) => b.isOverBudget).length}</div>
								...
								<Card key={budget.id}>
									<CardHeader>
										<div className="flex justify-between items-start">
											<div>
												...
												<CardDescription>{formatRecurrence(budget.recurrence as BudgetRecurrence)} Budget</CardDescription>
											</div>
									...
									<CardContent className="space-y-4">
										...
										<div className="text-xs text-gray-500">
											<p className="mt-1">Starts from {formatToHumanReadableDate(budget.startDate, "EEE, MMM dd, yyyy")}</p>
										</div>
									</CardContent>
								</Card>
							);
