		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			<DialogContent className="max-w-lg">
				<DialogHeader>
					...
					<DialogDescription>{isEditing ? "Update your budget details" : "Set a new budget amount and period"}</DialogDescription>
				</DialogHeader>
				<FormProvider {...form}>
					<form id="budget-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-1">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<SelectField name="category" label="Category" placeholder="Select a category" options={categoryOptions} required />
						</div>

						{/* Amount and Currency in one line */}
						<div className="flex gap-2">
							<div className="flex-1">
								<InputField name="amount" label="Amount" type="number" placeholder="0.00" min={0} step={0.01} required />
							</div>
							<div className="flex-1">
								<SelectField name="currency" label="Currency" placeholder="Currency" options={currencyOptions} required />
							</div>
						</div>
						...
						{showExchangeRate && (
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<InputField name="fromRate" label="From Rate" type="number" placeholder="1.00" min={0} step={0.01} required />
								<InputField name="toRate" label="To Rate" type="number" placeholder="1.00" min={0} step={0.01} required />
							</div>
						)}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<DateField name="startDate" label="Start Date" placeholder="Pick a date" source="budget" required />
						</div>
