...
import { TransactionType, ExpenseCategory, expenseOnboardingFormSchema, type ExpenseOnboardingFormData, BudgetProgress } from "@expense-tracker/shared-types";
		...
		<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
				...
				<div className="flex gap-3">
					...
					<div className="text-sm text-slate-900">
						...
						<p className="text-slate-700">
							Logging expenses helps you see where your money goes and stay within your budget. The more you track, the better insights you'll get!
						</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
				...
				<div>
					...
					<Controller name="title" control={control} render={({ field }) => <Input {...field} id="title" placeholder="e.g., Grocery shopping" className="mt-1" />} />
				...
				<div>
					...
					{selectedCategory && <p className="text-xs text-gray-500 mt-1">💡 Example: {categoryOptions.find((opt) => opt.value === selectedCategory)?.example}</p>}
				</div>
				...
				<div className="grid grid-cols-2 gap-4">
					...
					<div>
						...
						<Controller
							name="amount"
							control={control}
							render={({ field }) => <Input {...field} id="amount" type="number" step="0.01" placeholder="0.00" className="mt-1" />}
						/>
					...
					<div>
						...
						<Controller name="date" control={control} render={({ field }) => <Input {...field} id="date" type="date" className="mt-1" />} />
