...
const Step3FirstBudget = ({ onNext, onBack, onBudgetCreated, initialFormData, onFormDataChange, existingBudget }: Step3FirstBudgetProps) => {
			...
			<div className="bg-green-50 border border-green-200/50 rounded-lg p-4 mb-6">
				<div className="flex gap-3">
					...
					<div className="text-sm text-slate-900">
						...
						<p className="text-slate-700">Budgets help you control spending and reach your financial goals. We'll notify you when you're close to your limit!</p>
					</div>
				</div>
			</div>
			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
					...
					<div className="flex items-center gap-2 mb-1">
						...
						<TooltipProvider>
							<Tooltip>
								...
								<TooltipContent>
									<p className="w-48">Choose how often your budget resets (e.g., monthly budgets reset each month)</p>
								</TooltipContent>
							</Tooltip>
						</TooltipProvider>
					</div>
				...
				<div>
					...
					<Controller
						name="amount"
						control={control}
						render={({ field }) => <Input {...field} id="amount" type="number" step="0.01" placeholder="Enter budget amount" className="mt-1" />}
					/>
					{errors.amount && <p className="text-sm text-red-600 mt-1">{errors.amount.message}</p>}
					{selectedCategory && (
						<p className="text-xs text-gray-500 mt-1">
							💡 Suggested amount: {user?.currencySymbol || user?.currency || "$"} {categoryOptions.find((opt) => opt.value === selectedCategory)?.suggestion}
						</p>
					)}
				</div>
