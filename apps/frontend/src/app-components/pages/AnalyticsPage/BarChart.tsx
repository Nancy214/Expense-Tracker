...
const BarChartComponent: React.FC<BarChartProps> = ({
	...
	const generateInsights = (data: BarChartData[]): string[] => {
		...
		if (netIncome > 0) {
			...
			insights.push(`⚠️ Your expenses exceed income by ${formatAmount(Math.abs(netIncome))} across all months. Consider reviewing your spending.`);
		}

		if (totalIncome > 0 && totalExpense > 0) {
			const incomeExpenseRatio: number = totalIncome / totalExpense;
			if (incomeExpenseRatio < 1.2) {
				insights.push(`📊 Your income is only ${(incomeExpenseRatio * 100).toFixed(1)}% of expenses. Aim for higher savings.`);
			} else {
				insights.push(`✅ Great job! Your income is ${(incomeExpenseRatio * 100).toFixed(1)}% of expenses, maintaining good financial health.`);
			}
		}
		...
		const bestMonth: MonthData = monthsWithNet.reduce((best, current) => (current.net > best.net ? current : best), monthsWithNet[0]);
		const worstMonth: MonthData = monthsWithNet.reduce((worst, current) => (current.net < worst.net ? current : worst), monthsWithNet[0]);

		if (bestMonth.income > 0 || bestMonth.expense > 0) {
			insights.push(`📈 "${bestMonth.month}" was your best month with net income of ${formatAmount(bestMonth.net)}.`);
		}
		...
		if (months.length > 1) {
			...
			insights.push(`📊 Average monthly income: ${formatAmount(avgIncome)}, Average monthly expenses: ${formatAmount(avgExpense)}.`);
		}
				...
				<div className="mt-2 mb-2 text-xs sm:text-sm text-muted-foreground text-center px-2">
					{data.length > 0
						? (() => {
								...
								return `Average ${periodLabel} Income: ${formatAmount(avgIncome)} | Average ${periodLabel} Expenses: ${formatAmount(avgExpense)}`;
							})()
						: "No data available."}
				</div>
