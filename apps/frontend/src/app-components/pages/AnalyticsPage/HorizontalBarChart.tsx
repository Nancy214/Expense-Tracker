...
const generateInsights = (data: { name: string; value: number }[]): string[] => {
	...
	if (topCategoryPercentage > 40) {
		insights.push(`🔴 Your highest category "${topCategory.name}" represents ${topCategoryPercentage.toFixed(1)}% of total. Consider diversifying.`);
	}

	// Multiple high categories
	if (highCategories.length > 2) {
		insights.push(`⚠️ You have ${highCategories.length} categories each representing over 20%. Review your allocation.`);
	}
	...
	if (expensiveCategories.length > 0) {
		insights.push(`💰 ${expensiveCategories.length} categories are significantly above average. Review for potential savings.`);
	}
					...
					{data.map((segment) => (
						<div key={segment.name} className="flex items-center justify-between">
							<div className="flex items-center gap-3">
								...
								<span className="text-sm font-medium text-gray-900 dark:text-gray-100">{segment.name}</span>
							</div>
							<div className="text-right">
								<div className="text-sm font-semibold text-gray-900 dark:text-gray-100">{segment.percentage.toFixed(1)}%</div>
								<div className="text-xs text-gray-500 dark:text-gray-400">{formatAmount(segment.amount, currencySymbol)}</div>
							</div>
						</div>
					))}
				</div>
