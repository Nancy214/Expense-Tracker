...
const QuickInsights: React.FC<{
    ...
    const healthByCurrency: Record<
        string,
        { totalIncome: number; totalExpenses: number; savingsRate: number; isHealthy: boolean }
    > = {};
    currencies.forEach((currency) => {
            ...
            <div className={`grid gap-4 ${currencies.length > 1 ? "grid-cols-1 md:grid-cols-2" : "grid-cols-1"}`}>
                {currencies.map((currency) => {
                    ...
                    return (
                        <Card
                            key={currency}
                            className="rounded-xl shadow-lg border-0 bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800"
                        >
                            <CardContent className="p-6">
                ...
                {currencies.map((currency) => {
                    ...
                    return (
                        ...
                        >
                            <CardContent className="p-6">
                                <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">
                                    Where is your money going? ({currency})
                                </h3>
...
const AnalyticsPage = () => {
    ...
    const { isLoading: expenseBreakdownLoading, error: expenseBreakdownError } = useExpenseCategoryBreakdown(
        selectedPeriod,
        selectedSubPeriod
    );

    const { isLoading: billsBreakdownLoading, error: billsBreakdownError } = useBillsCategoryBreakdown(
        selectedPeriod,
        selectedSubPeriod
    );

    const { isLoading: incomeExpenseLoading, error: incomeExpenseError } = useIncomeExpenseSummary(
        selectedPeriod,
        selectedSubPeriod
    );

    const { isLoading: savingsTrendLoading, error: savingsTrendError } = useMonthlySavingsTrend(
        selectedPeriod,
        selectedSubPeriod
    );
