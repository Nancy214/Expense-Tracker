...
const transformPieDataToBarData = (pieData: HorizontalBarData[], colors: string[] = ["#4F7FFF", "#10B981", "#F59E0B", "#8B5CF6", "#06B6D4"]) => {
...
const QuickInsights: React.FC<{
	...
	const totalIncome = incomeExpenseData.filter((item) => item.category === TransactionType.INCOME).reduce((sum, item) => sum + item.value, 0);
	const totalExpenses = incomeExpenseData.filter((item) => item.category === TransactionType.EXPENSE).reduce((sum, item) => sum + item.value, 0);
	...
	return (
		<div className="space-y-4">
			...
			<Card className="rounded-xl shadow-lg border-0 bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800">
				<CardContent className="p-6">
					<div className="flex items-start justify-between mb-4">
						<div>
							<h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">This month's financial health</h3>
						</div>
						{isHealthy ? (
					...
					<div className="space-y-2">
						<div className="flex justify-between text-sm">
							<span className="text-gray-600 dark:text-gray-400">Saving {savingsRate.toFixed(0)}% of income</span>
							<span className="text-gray-600 dark:text-gray-400">Target: 20%</span>
						</div>
						<div className="relative h-4 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
							<div
								className={`absolute top-0 left-0 h-full transition-all duration-500 ${
									isHealthy ? "bg-gradient-to-r from-green-400 to-green-600" : "bg-gradient-to-r from-amber-400 to-amber-600"
								}`}
								style={{ width: `${Math.min(savingsRate, 100)}%` }}
							/>
							{/* Target line at 20% */}
							<div className="absolute top-0 h-full w-0.5 bg-gray-600 dark:bg-gray-400" style={{ left: "20%" }} />
						</div>
			...
			<Card className="rounded-xl shadow-lg border-0 bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-800">
				<CardContent className="p-6">
					<h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Where is your money going?</h3>
...
const AnalyticsPage = () => {
	...
	const { data: expenseBreakdown, isLoading: expenseBreakdownLoading, error: expenseBreakdownError } = useExpenseCategoryBreakdown(selectedPeriod, selectedSubPeriod);

	const { data: billsBreakdown, isLoading: billsBreakdownLoading, error: billsBreakdownError } = useBillsCategoryBreakdown(selectedPeriod, selectedSubPeriod);

	const { data: incomeExpenseResponse, isLoading: incomeExpenseLoading, error: incomeExpenseError } = useIncomeExpenseSummary(selectedPeriod, selectedSubPeriod);

	const { data: savingsTrendResponse, isLoading: savingsTrendLoading, error: savingsTrendError } = useMonthlySavingsTrend(selectedPeriod, selectedSubPeriod);
	...
	return (
		<div className="p-3 sm:p-4 md:p-6 lg:p-4 max-w-7xl mx-auto space-y-4 sm:space-y-6">
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
				...
				<Button variant="outline" size="sm" onClick={() => setShowAdvancedView(!showAdvancedView)} className="flex items-center gap-2">
					{showAdvancedView ? (
						<>
