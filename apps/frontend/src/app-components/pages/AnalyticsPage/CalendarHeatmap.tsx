...
const CalendarHeatmapComponent: React.FC<CalendarHeatmapProps> = ({ title, description, data, colorScale = DEFAULT_COLORS, showLegend = true, maxValue, year }) => {
				...
				<CardHeader className="p-4 sm:p-6">
					<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
						<div>
							...
							{description && <CardDescription className="text-xs sm:text-sm text-muted-foreground mt-1">{description}</CardDescription>}
						</div>
					...
					{showLegend && (
						<div className="flex items-center justify-center space-x-1 text-xs sm:text-sm text-muted-foreground">
							...
							{colorScale.slice(1).map((color: string, index: number) => (
								<div key={`${color}-${index}`} className="w-3 h-3 sm:w-4 sm:h-4 rounded-sm" style={{ backgroundColor: color }} />
							))}
							<span>More</span>
						</div>
					)}
