...
const PieChartComponent: React.FC<PieChartProps> = ({
    ...
    // Ensure data is sorted by value in descending order
        ...
        <div className="bg-white dark:bg-slate-900/80 rounded-xl sm:rounded-2xl shadow-lg p-2 sm:p-3 md:p-4 transition hover:shadow-2xl">
            ...
            <div className="w-full flex flex-col items-center">
                    ...
                    <div className="w-full h-[150px] sm:h-[200px] md:h-[240px] lg:h-[280px] xl:h-[320px]">
                        <ResponsiveContainer width="100%" height="100%">
                            ...
                            >
                                ...
                                >
                                    {data.map((item, idx) => (
                                        <Cell key={`cell-${item.name}`} fill={chartColors[idx % chartColors.length]} />
                                    ))}
                                </Pie>
                                <Tooltip
                ...
                <div className="mt-2 mb-2 text-xs sm:text-sm text-muted-foreground text-center px-2">
                    {data.length > 0
                        ? (() => {
                              const topCategory = data.slice().sort((a, b) => b.value - a.value)[0];
