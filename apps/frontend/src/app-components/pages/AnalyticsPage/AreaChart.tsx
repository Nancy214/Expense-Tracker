...
const AreaChartComponent: React.FC<AreaChartProps> = ({
	...
	const generateInsights = (data: AreaChartData[]): string[] => {
		...
		// Overall performance
		if (totalSavings > 0) {
			insights.push(`💰 You've saved a total of ${formatAmount(totalSavings)} over the last ${monthCount} month${monthCount !== 1 ? "s" : ""}!`);
		} else {
			insights.push(`📊 Your total savings over the last ${monthCount} month${monthCount !== 1 ? "s" : ""} is ${formatAmount(totalSavings)}.`);
		}
		...
		if (avgSavings > 0) {
			...
			insights.push(`⚠️ Your average monthly savings is ${formatAmount(avgSavings)}. Consider reviewing your spending habits.`);
		}

		// Find best and worst months
		const bestMonth: AreaChartData = data.reduce((best, current) => (current.savings > best.savings ? current : best), data[0]);
		const worstMonth: AreaChartData = data.reduce((worst, current) => (current.savings < worst.savings ? current : worst), data[0]);
		...
		if (recentAvg > avgSavings) {
			insights.push(`🚀 Your recent 3-month average (${formatAmount(recentAvg)}) is above your overall average - great momentum!`);
		} else if (recentAvg < avgSavings) {
			insights.push(`📉 Your recent 3-month average (${formatAmount(recentAvg)}) is below your overall average - consider adjusting your strategy.`);
		}
		...
		if (totalIncome > 0 && totalExpenses > 0) {
			...
			if (savingsRate > 20) {
				...
				insights.push(`⚠️ You're saving ${savingsRate.toFixed(1)}% of your income. Consider increasing your savings rate.`);
			}
		}
		...
		if (positiveMonths > negativeMonths) {
			insights.push(`✅ You had ${positiveMonths} positive months vs ${negativeMonths} negative months - great consistency!`);
		} else {
			insights.push(`⚠️ You had ${negativeMonths} negative months vs ${positiveMonths} positive months - focus on improving your savings.`);
		}
						...
						<div className="space-y-1 sm:space-y-2">
							{formattedData.length > 0 ? (
								...
								<div className="text-xs text-muted-foreground rounded">
									Start tracking your income and expenses to see personalized insights about your savings patterns.
								</div>
							)}
						</div>
					</div>
