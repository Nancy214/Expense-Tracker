...
export function arrayToCSV(data: Transaction[]): string {
	...
	const csv = [
		...
		...data.map((row) =>
			header
				.map((fieldName) => {
					if ((fieldName === "fromRate" || fieldName === "toRate") && (row as any).fromRate === 1 && (row as any).toRate === 1) {
						return "";
					}
...
export function generateMonthlyStatementPDF({
	...
	// Add total row
	const transactionRowsWithTotal = [...transactionRows, ["", "Total", "", `${userCurrency} ${totalAmount.toFixed(2)}`, ""]];
