			...
			{onPageChange && (
				<div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
					<div className="text-sm text-muted-foreground">
						Showing {(currentPage - 1) * itemsPerPage + 1} to {(currentPage - 1) * itemsPerPage + data.length} of {totalItems} results
					</div>

					{totalPages > 1 && <PaginationWrapper currentPage={currentPage} totalPages={totalPages} onPageChange={onPageChange} isLoading={isLoading} />}
				</div>
			)}
		</>
	);
