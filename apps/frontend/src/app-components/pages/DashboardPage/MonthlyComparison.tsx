...
const MonthlyComparison = ({ monthlyStats, monthlyStatsByCurrency, previousMonthExpenses, previousMonthExpensesByCurrency, formatAmount, isLoading }: MonthlyComparisonProps) => {
	const navigate = useNavigate();

	// Render multi-currency amounts for current month
	const renderMultiCurrencyAmount = () => {
		if (!monthlyStatsByCurrency) {
			return <p className="text-2xl font-bold text-gray-900 dark:text-white">{formatAmount(monthlyStats.totalExpenses)}</p>;
		}
	...
	const calculateChangeByCurrency = () => {
		if (!monthlyStatsByCurrency || !previousMonthExpensesByCurrency) {
			const expenseChange = previousMonthExpenses > 0 ? ((monthlyStats.totalExpenses - previousMonthExpenses) / previousMonthExpenses) * 100 : 0;
		...
		<Card>
			<CardContent className="p-6">
				<div className="flex items-center justify-between mb-4">
					...
					<Button variant="ghost" size="sm" className="text-xs" onClick={() => navigate("/analytics", { state: { showAdvanced: true } })}>
						Details
										...
										<>
											<div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
												<div className="flex items-center gap-2">
													<div
														className={`p-1.5 rounded-full ${
															isIncrease && isSignificantChange ? "bg-red-100 dark:bg-red-900/30" : "bg-gray-100 dark:bg-gray-700/50"
														}`}
													>
														{isIncrease ? (
															<TrendingUp className={`h-4 w-4 ${isSignificantChange ? "text-red-600" : "text-gray-600 dark:text-gray-400"}`} />
														) : (
															<TrendingDown className={`h-4 w-4 ${isSignificantChange ? "text-emerald-600" : "text-gray-600 dark:text-gray-400"}`} />
														)}
													</div>
													<div>
														<p className="text-sm font-medium text-gray-900 dark:text-white">vs. Last Month</p>
												...
												<div className="text-right">
													...
													<p className="text-xs text-muted-foreground">{formatAmount(Math.abs(monthlyStats.totalExpenses - previousMonthExpenses))}</p>
												</div>
											</div>
										</>
									);
								...
								return (
									<>
										{currencies.map((currency) => {
											const current = monthlyStatsByCurrency[currency].expense;
											const previous = previousMonthExpensesByCurrency?.[currency] || 0;
											const expenseChange = previous > 0 ? ((current - previous) / previous) * 100 : 0;
											const isIncrease = expenseChange > 0;
											const isSignificantChange = Math.abs(expenseChange) >= 5;
													...
													<div className="flex items-center gap-2">
														...
														>
															{isIncrease ? (
																...
																<TrendingDown
																	className={`h-4 w-4 ${isSignificantChange ? "text-emerald-600" : "text-gray-600 dark:text-gray-400"}`}
																/>
															)}
														</div>
														<div>
															<p className="text-sm font-medium text-gray-900 dark:text-white">vs. Last Month ({currency})</p>
															<p className="text-xs text-muted-foreground">{formatAmount(previous, currency)}</p>
														</div>
													</div>
													<div className="text-right">
														<p
														...
														<p className="text-xs text-muted-foreground">{formatAmount(Math.abs(current - previous), currency)}</p>
													</div>
												</div>
											);
										})}
