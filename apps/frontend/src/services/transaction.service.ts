...
export const getExpenses = async (page: number = 1, limit: number = 20): Promise<{ expenses: Transaction[]; pagination: PaginationInfo }> => {
	try {
...
export const getMonthlyStats = async (): Promise<MonthlyStats> => {
	try {
		...
		// Calculate totals
		const totalIncome = monthlyExpenses.filter((expense: Transaction) => expense.type === "income").reduce((sum: number, expense: Transaction) => sum + expense.amount, 0);

		const totalExpenses = monthlyExpenses.filter((expense: Transaction) => expense.type === "expense").reduce((sum: number, expense: Transaction) => sum + expense.amount, 0);
