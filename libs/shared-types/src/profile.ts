...
export const ZProfileData = z.object({
	name: z
		...
		.trim(),
	email: z.email("Invalid email address").max(MAX_EMAIL_LENGTH, `Email must be less than ${MAX_EMAIL_LENGTH} characters`).toLowerCase().trim(),
	phoneNumber: z
		.string()
		.transform((val) => (val === "" ? undefined : val))
		.optional()
		.refine((val) => !val || /^\d+$/.test(val), "Phone number must contain only numbers")
		.refine((val) => !val || val.length <= MAX_PHONE_LENGTH, `Phone number must be less than ${MAX_PHONE_LENGTH} characters`),
	dateOfBirth: z
		.string()
		.transform((val) => (val === "" ? undefined : val))
		.optional()
	...
	currency: z
		...
		.refine((val) => /^[A-Z]{3}$/.test(val) || val.length <= 3, "Choose a valid currency"),
	currencySymbol: z
		.string()
		.transform((val) => (val === "" ? undefined : val))
		.optional(),
