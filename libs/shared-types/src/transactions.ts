...
export const ZTransaction = z.object({
	...
	date: z.string({ message: "Date is required" }).min(1, "Date is required").refine(isValidDate, "Please enter a valid date in DD/MM/YYYY format"),
	title: z.string({ message: "Title must be a string" }).max(50, "Title must be less than 50 characters").trim().optional().or(z.literal("")),
	amount: z.number({ message: "Amount must be a number" }).positive("Amount must be greater than 0").max(999999999, "Amount cannot exceed 999,999,999"),
	description: z.string().max(200, "Description must be less than 200 characters").optional().or(z.literal("")),
	category: z.string(),
	currency: z.string({ message: "Currency is required" }).min(1, "Currency is required").max(10, "Currency code is too long"),
	type: z.enum([TransactionType.INCOME, TransactionType.EXPENSE] as const, {
		message: "Transaction type must be either income or expense",
	}),
	fromRate: z.number({ message: "Exchange rate must be a number" }).positive("Exchange rate must be greater than 0").default(1).optional(),
	toRate: z.number({ message: "Exchange rate must be a number" }).positive("Exchange rate must be greater than 0").default(1).optional(),
	userId: z.string(),
	receipt: z
		.union([
			z
				.instanceof(File)
				.refine((file: File) => file.size <= MAX_FILE_SIZE, `File size must be less than ${MAX_FILE_SIZE / (1024 * 1024)}MB`)
				.refine((file: File) => VALID_FILE_TYPES.includes(file.type as (typeof VALID_FILE_TYPES)[number]), `File type must be one of: ${VALID_FILE_TYPES.join(", ")}`),
	...
	recurringEndDate: z.string().min(1).refine(isValidDate, "Please enter a valid date in DD/MM/YYYY format").optional(),
